...
int s21_sprintf(char *buffer, const char *format, ...) {
  ...
  va_end(vaList);
  return (something_goes_wrong) ? -1 : buffer - buffer_start;
}
...
char *s21_widthFlag(char *digit_str, s21_specifiers *s) {
  ...
  if ((s->flags.minus))
    insert_pos = digit_len;
           ...
           (s->flags.zero))
    insert_pos = 1;
  char *insertion = s21_insert_stat(digit_str, str_w, insert_pos);
...
char *s21_precFlag(char *digit_str, s21_specifiers *s) {
  ...
  char *insertion = s21_insert_stat(digit_str, str_p, insert_pos);
  if (insertion) s21_strcpy(digit_str, insertion);
...
int s21_handle_NaN_INF(char *buf, int sign, long double value,
  ...
  if (res && s21_strchr("FEG", s->types)) {
    s21_strcpy(buf, s21_to_upper_stat(buf));
  }
  return res;
}
...
char *printb_d(char *dest, unsigned long value, s21_specifiers *s) {
  ...
  if (s->types == 'X') s21_strcpy(tmp_digit, s21_to_upper_stat(tmp_digit));

  if (s->flags.sharp) {  //!!
    if (radix == 8) {
      s21_strcpy(tmp_digit, s21_insert_stat(tmp_digit, "0", 0));
      sign = 1;
    } else if (radix == 16 && value != 0) {
      s21_strcpy(
          tmp_digit,
          s21_insert_stat(tmp_digit, ((s->types == 'X') ? "0X" : "0x"), 0));
      sign = 2;
      if (s->flags.zero &&
          s->widths >=
